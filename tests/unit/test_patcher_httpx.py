...
def test_gzip_header_with_plain_body_replay(tmp_path):
    file_path = tmp_path / "dvd_gzip.json"

    # Seed a DVD with a response that has Content-Encoding: gzip but a plain (decoded) body
    from dvd_rw.models import Request, Response
    ...
    with DVDLoader(
        ...
        assert not dvd.from_file
        req = Request(headers=[], method="GET", url=make_url("/gzip"))
        res = Response(
            status=200,
            headers=[
    ...
    # Now load in replay mode and ensure we can construct httpx.Response and read content
    def fail_handler(request):  # pragma: no cover - should not be called in replay
        raise AssertionError("Network should not be hit during replay")
    ...
    with DVDLoader(
        ...
        assert dvd2.from_file
        client = httpx.Client(transport=httpx.MockTransport(fail_handler))
        resp = client.get(make_url("/gzip"))
        assert resp.status_code == 200
        assert resp.content == b"hello"
        # Header should be preserved on the replayed response
        assert resp.headers.get("Content-Encoding") == "gzip"


def test_client_base_url_and_headers_recorded(tmp_path):
    file_path = tmp_path / "dvd_base_headers.json"

    transport = httpx.MockTransport(lambda req: httpx.Response(200, content=b"ok"))

    base_url = f"{BASE_URL}/api/"
    client_headers = {"X-Client": "yes", "Authorization": "Token 123"}
    ...
    with DVDLoader(
        ...
        resp = client.get("v1/items?x=1", headers={"X-Req": "1"})
        assert resp.status_code == 200
        ...
        assert headers_lower.get("authorization") == "Token 123"
        assert headers_lower.get("x-req") == "1"
        # Also ensure host derived from URL is correct
        assert recorded_req.host == "example.com"
